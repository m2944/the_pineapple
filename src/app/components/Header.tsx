...
export default function Header() {
  ...
  return (
    // We'll control the background from globals.css, so removing bg-black
    <div className="w-full"> 
      <header 
        style={{ color: 'var(--header-text-color)' }} // Applying the specific text color
        className="w-full max-w-5xl mx-auto flex justify-between items-center py-3 px-4 md:px-0 relative z-50 font-[var(--font-neue)] text-[14px] font-medium" // Applying new font, size, and weight
      >
        <Link href="/">
          <h1 className="text-2xl font-bold text-white">The Pineapple</h1>
        </Link>
        <nav className="hidden md:flex items-center space-x-8">
          ...
          <button className="hover:text-white cursor-pointer">Restaurants</button>
          <button className="hover:text-white cursor-pointer">Food</button>

        </nav>
        <button className="bg-green-600 text-white px-4 py-2 rounded-full font-semibold hover:bg-green-700 transition-colors flex items-center gap-2">
          <span>Get Olive</span>
