...
export default function Header() {
  ...
  return (
    <header className="w-full max-w-5xl mx-auto text-[oklch(0.44_0.005_-0.016)] flex justify-between items-center py-4 px-4 md:px-0 relative z-50 text-sm font-medium">
      <Link href="/">
        <h1 className="text-2xl font-bold text-green-800">The Pineapple</h1>
      </Link>
      <nav className="hidden md:flex items-center space-x-8">
        ...
        >
          <span className="hover:text-black">Solutions</span>
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4">
              <div className="bg-white rounded-xl shadow-lg p-6 grid grid-cols-2 gap-x-12 gap-y-4 w-[768px]">
                ...
                <Link href="/blog" onClick={() => setIsSolutionsOpen(false)} className="block relative w-full h-full rounded-lg overflow-hidden group">
                  <Image src="/family_feeding.jpg" alt="Blog promo" fill style={{ objectFit: 'cover' }} className="group-hover:scale-105 transition-transform duration-300" />
        ...
        <button className="hover:text-black cursor-pointer">Restaurants</button>
        <button className="hover:text-black cursor-pointer">Food</button>

      </nav>
      <button className="bg-[oklch(0.47_0.13_141.5)] text-white px-4 py-2 rounded-full font-semibold hover:opacity-90 transition-opacity flex items-center gap-2 text-sm">
        <span>Get Olive</span>
